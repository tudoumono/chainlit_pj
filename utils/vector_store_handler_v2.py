"""
ベクトルストア管理モジュール（V2）
OpenAI beta.vector_stores APIの廃止に対応したローカル管理版
"""
...
class VectorStoreHandler:
        ...
        try:
            ...
            vs_data = {
                ...
                "file_counts": {
                    ...
                    "in_progress": 0,
                    "completed": len(file_ids) if file_ids else 0,
                    "failed": 0
                }
            }
            
            # JSONファイルとして保存
            vs_file = os.path.join(self.vs_dir, f"{vs_id}.json")
            with open(vs_file, "w", encoding="utf-8") as f:
                json.dump(vs_data, f, indent=2, ensure_ascii=False)
            
            print(f"✅ ベクトルストア作成成功: {vs_id}")
            print(f"   名前: {name}")
            print(f"   ファイル数: {len(file_ids) if file_ids else 0}")
    ...
    async def upload_file(self, file_path: str, purpose: str = "assistants") -> Optional[str]:
        ...
        try:
            ...
            file_info_path = os.path.join(self.files_dir, f"{response.id}.json")
    ...
    async def add_file_to_vector_store(self, vector_store_id: str, file_id: str) -> bool:
        ...
        """
        try:
            vs_file = os.path.join(self.vs_dir, f"{vector_store_id}.json")
            ...
            if file_id not in vs_data["file_ids"]:
                ...
                vs_data["file_counts"]["total"] = len(vs_data["file_ids"])
                vs_data["file_counts"]["completed"] = len(vs_data["file_ids"])
    ...
    async def list_vector_stores(self) -> List[Dict]:
        ...
        try:
            ...
            if os.path.exists(self.vs_dir):
                for vs_file in Path(self.vs_dir).glob("*.json"):
                    try:
                        with open(vs_file, "r", encoding="utf-8") as f:
                            vs_data = json.load(f)
                        ...
                        stores_list.append({
                            "id": vs_data["id"],
                            "name": vs_data["name"],
                            "file_counts": vs_data.get("file_counts", {"total": len(vs_data.get("file_ids", []))}),
                            "created_at": datetime.fromisoformat(vs_data["created_at"]).timestamp(),
                            "status": vs_data.get("status", "completed")
                        })
                    except Exception as e:
            ...
            for file_id in vs_data.get("file_ids", []):
                ...
                if os.path.exists(file_info_path):
                    ...
                    files_list.append({
                        ...
                        "created_at": datetime.fromisoformat(file_info.get("created_at", datetime.now().isoformat())).timestamp(),
                        "status": "processed"
                    })
                else:
                    files_list.append({
...
"""
    ...
    def format_file_list(self, files: List[Dict]) -> str:
        ...
        for i, file_info in enumerate(files, 1):
            formatted += f"{i}. 📄 {file_info.get('filename', 'ID: ' + file_info['id'])}\n"
